...
class RaisingFacilityController {
  ...
  store(req, res, next) {
    const { id, name, livestock_type, owner, location, size, employee_number, certificate_ids } = req.body;

    const raisingFacilityId = parseInt(id, 10);
    const employeeNumber = parseInt(employee_number, 10);

    if (!raisingFacilityId || !name || !livestock_type || !owner || !location || !size || !employeeNumber) {
  ...
  edit(req, res, next) {
    Promise.all([
      ...
      .then(([raisingFacility, certificates]) => {
        ...
        const selectedCertificates = raisingFacilityObject.certificates.map((cert) => cert.id);
  ...
  update(req, res, next) {
    const { name, livestock_type, owner, location, size, employee_number, certificate_ids } = req.body;
    const raisingFacilityId = parseInt(req.params.id, 10);

    if (!raisingFacilityId || !name || !livestock_type || !owner || !location || !size || !employee_number) {
      return res.status(400).json({ message: "Các thông tin không được để trống." });
    }
